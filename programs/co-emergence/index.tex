...
$0 = x^{\dagger}(\rho+\rho^{\mathsf T})x
   = x^{\dagger}\rho\,x + x^{\dagger}\rho^{\mathsf T}x$,
and both summands are nonnegative --- $\rho$ by hypothesis, and
$\rho^{\mathsf T}$ because it is isospectral to $\rho$ and hence positive
semidefinite, as used in part~(a). A sum of two nonnegative reals vanishes
only if each does, so in particular $x^{\dagger}\rho\,x = 0$ and therefore
$\rho x = 0$; thus
...
\begin{theorem}[Freedman, 1982; Donaldson, 1983; Taubes,
1987~\cite{freedman,donaldson,taubes}; Stallings, 1962~\cite{stallings}]
\emph{(Rigorous.)}
\label{thm:exotic}
The smooth manifold $\mathbb{R}^4$ admits uncountably many mutually
non-diffeomorphic smooth structures~\cite{freedman,donaldson,taubes}. In every
other dimension $n \neq 4$, $\mathbb{R}^n$ admits a unique smooth structure up
to diffeomorphism~\cite{stallings}.
\end{theorem}

\noindent Both halves need care about attribution. Existence of even a single
exotic $\mathbb{R}^4$ requires Freedman's classification~\cite{freedman}
\emph{together with} Donaldson's diagonalizability theorem~\cite{donaldson} ---
the ``Freedman--Donaldson machinery'' referred to elsewhere in this paper ---
with Taubes~\cite{taubes} then supplying the uncountable family. The
$n \neq 4$ uniqueness half is a separate result: for $n \geq 5$ it is due to
Stallings~\cite{stallings}, and for $n \leq 3$ to classical low-dimensional
topology. It is \emph{not} a consequence of Smale's $h$-cobordism
theorem~\cite{smale}, which concerns compact manifolds and is invoked for that
distinct purpose elsewhere in this paper.
...
\textit{J.\ Math.\ Phys.} \textbf{11}, 437--449 (1970).
...
\textit{Proc.\ Cambridge Philos.\ Soc.} \textbf{58}, 481--488 (1962).
