...
4. **M3 ≡ M8.** A genuine spatial reduction of the response kernel would also
   resolve M8 (the conflation of a spacetime fixed point of `F` with a fixed
   point of data on a chosen Σ, whose gauge-equivalence is never constructed).
   Until Σ is shown pure gauge, no spatial-resolvent bound is licensed; the two
   gaps are one obstruction seen from two sides.
...
`D = 0` (dimensionless). Ratio displayed/literal `= (ℓ_P²R)²`; they agree iff
`R = ℓ_P⁻²` — the silent Planck-curvature substitution, applied once per factor
of `ℓ_P²R`.
...
different κ:
- **`μ²~m²` (Compton):** `C_inv ~ 1/m²`, `‖K^red‖ ~ ħm²/(4π)²` ⟹ **`κ ~ (m/M_P)²`,
  R-independent.** Flat-space limit `R → 0` graceful (κ unaffected); no lower
  bound on R needed. Matches the mass table in §3. This is the honest headline.
...
choice that collides with M4.** Recommend a joint M1+M4 treatment; the corrected
...
foliation," local-in-time, not the general Theorem 2. M3 ≡ M8: the same work
resolves the well-definedness gap. Recorded as an observation; OBJECTIVES edits
are the governor's.
