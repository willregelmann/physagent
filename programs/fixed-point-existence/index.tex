...
operator $(\Box_g + m^2 + \xi R)$, whose support is the causal past $J^-(x)$; this is an
operator-support property, independent of the field mass (the compact-$\Sigma$ spectral
...
dependence is required. This is the same fork the (M1) resolution reaches from the
...
on~$R$ is needed). This reproduces the mass table below and is the defensible headline.
...
with $f\to\mathrm{const}$ for $\mu^2\sim m^2$ and $f\sim\ell_P^2 R$ for $\mu^2\sim R$. The
...
operator on a single Cauchy surface $\Sigma$ exists within (A1)--(A6) (the same gap as
(M8); the obstruction is mass-independent, so it is not cured in the massless limit). The
...
Theorem~\ref{thm:banach}. \emph{(M1) is a hidden-reference-scale artifact} (resolved at
Sketch level in Section~\ref{sec:banach}): with a single explicit Sobolev scale $\mu$ the
$\kappa$ chain is consistent and gives $\kappa\sim(m/M_P)^2$ ($R$-independent) for
$\mu^2\sim m^2$, or $\kappa\sim\ell_P^2 R$ for $\mu^2\sim R$ --- the latter forcing the
local scale $\hbar R/(4\pi)^2$, i.e.\ \emph{(M4)}, so (M1) and (M4) are one joint
repair. \emph{(M5)} (the self-mapping/displacement bound) remains a genuine technical gap
