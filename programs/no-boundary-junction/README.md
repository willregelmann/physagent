...
- `notes/2026-09-06-j6-saddle-commitment-and-orientation.md` — J-6: the two definitions of the no-boundary state on record give opposite orderings of the two real caps (Hartle–Hawking sign: double bubble dominates above ε_c; tunneling sign: sphere always); the Robin / Euclidean-momentum definition admits both caps, so under the program's minimum-excitation premise J-1's bound stands. Commitment: the Hartle–Hawking sign. Orientation: the tree re-expresses the branch's causal order and the state's record arrow, deriving neither; root uniqueness is a dominance statement (Sketch throughout; the fourth-order Picard–Lefschetz analysis is the named open computation).
...
| Under the Robin / Euclidean-momentum definition both real caps contribute with the Hartle–Hawking sign, so J-1's bound is the program's result; under the real-lapse tunneling sign the ordering inverts | Sketch (transcription of the Robin condition to the pole series; the fourth-order Picard–Lefschetz analysis is open) | J-6 note §1 |
