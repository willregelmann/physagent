...
| [`no-boundary-junction`](programs/no-boundary-junction/) | Self-consistency at a signature-change junction between a no-boundary Euclidean cap (the anomaly-driven fixed point) and Lorentzian branches. Established so far: the junction alone generates no scale beyond the cap's (a coupling fixed at the junction is what a mass hierarchy would need); the cap is semiclassical only for very large field content, stated as an input; the root's de Sitter depth is set by the free $\Box R$ coefficient, not by the fixed point. Experimenter-priority (#215). | Early notes |
